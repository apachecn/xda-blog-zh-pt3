# 2023 年惠普精英蜻蜓 G3 最佳配件

> 原文：<https://www.xda-developers.com/best-hp-elite-dragonfly-g3-accessories/>

惠普精英版蜻蜓 G3(T1)可能会成为今年 T2 最佳商务笔记本电脑(T3)之一。与其前代产品相比，它拥有更强大的处理器、新的 3:2 显示屏、更好的摄像头和更柔和的设计，这可能是一些商业用户更喜欢的。但是不管笔记本电脑有多棒，总有一些事情是你希望自己能做的。这就是配件的用武之地，我们已经为 HP Elite 蜻蜓 G3 收集了最好的配件。

你可以为你的笔记本电脑购买各种配件。从在旅途中保护笔记本电脑安全的外壳，到帮助您提高工作效率的第二台显示器，甚至是您在家时可以用来玩游戏的外部 GPU，我们在这里都有介绍。您可以使用下面的列表跳转到您感兴趣的特定类别。

**浏览这篇文章:**

## 惠普精英蜻蜓 G3 显示器

惠普 Elite 蜻蜓 G3 的显示屏非常有利于提高工作效率。高 3:2 的宽高比使它更容易完成工作，并且它需要更少的滚动。但是，即使有一个很棒的笔记本电脑显示屏，有第二个屏幕总是有帮助的。多显示器设置是多任务处理者所信赖的，它们允许完成更多的工作，而不必处理一堆打开的窗口和应用程序。显示器是惠普精英版蜻蜓 G3 的最佳配件之一，我们有一些建议:

*   <picture>![The HP U27 is a sharp 27-inch display with 4K resolution and it's great for getting work done.](img/365469b433dd94e54b4432720614b946.png)</picture>

    惠普 U27 4K 无线显示器

    ##### 惠普 U27 4K 无线显示器

    如果你想扩展你的工作空间，惠普 U27 是一款很棒的显示器。它有一个 27 英寸的大面板，并有非常清晰的 4K 分辨率，这也有助于更多的内容适合屏幕，并仍然看起来清晰。它有 HDMI 和 DisplayPort 输入，但如果你想减少桌子上的混乱，你也可以无线连接。

*   <picture>![Ultra-wide monitors are very popular with  multi-taskers, and the Samsung S65UA is a great one. It's a large 34-inch screen and the sharp UWQHD resolution makes it ideal for getting work done. This is a USB-C monitor, so it works with the Dell XPS 13 out of the box.](img/63caee19a672553ec91cb545efd50d2b.png)</picture>

    三星 S65UA

    ##### 三星 S65UA 超宽显示器

    超宽显示器是提高工作效率的最佳工具之一，因为它可以更容易地并排打开多个应用程序。此外，这款显示器具有清晰的 UWQHD 分辨率，支持 HDR10，并且具有 100Hz 的刷新率，因此一切看起来都很流畅。它通过 USB-C 连接，因此它也可以为您的笔记本电脑充电，并作为 USB 集线器。

*   <picture>![If you're in the market for a 4K monitor, this one from LG is a great choice. In addition to 4K resolution, it supports VESA DisplayHDR 400 and it covers 95% of DCI-P3\. Plus, it connects via HDMI, DisplayPort, or USB Type-C which can also deliver 96W of power to your laptop.](img/4d8a593f1586d5452887c3ec8d70e0ce.png)</picture>

    LG 27up 850-W

    ##### LG 27up 850-W

    如果你想买一台 4K 显示器，这款 LG 的显示器是个不错的选择。除了 4K 分辨率，它还支持 VESA DisplayHDR 400，并覆盖 95%的 DCI-P3。此外，它通过 HDMI、DisplayPort 或 USB Type-C 连接，也可以为您的笔记本电脑提供 96W 的功率。

*   <picture>![Want to dip into dual-screen setups without spending a lot? This HP monitor may be a great way to do it, giving you a Full HD panel with a 75Hz refresh rate, plus built-in speakers for a very low price. It doesn't connect via USB-C, so you may need an adapter for the smaller Spectre x360, but it does have features like tilt, height, and pivot adjustments. For this price, you can't ask for more.](img/057036a7b74a4fcb0ac53baa92bdde34.png)</picture>

    HP 24MH

    ##### HP 24MH FHD 显示器

    这不是最新的显示器，但如果你只是想要一台基本的显示器，它的表现出人意料地好。它拥有全高清分辨率，75Hz 的刷新率，体验略显流畅，甚至还包括内置扬声器，在这样一款经济实惠的显示器上观看真是太棒了。

*   <picture>![Once you're used to a dual-monitor setup, it can be hard to go back, so a portable monitor is a great way to get a similar experience anywhere. This 15.6-inch monitor has Full HD resolution and built-in speakers. It connects using a single USB-C cable so it's easy to set up, though it also has Mini HDMI.](img/21c8b32d7991152d3cd8038bffad8539.png)</picture>

    Lepow Z1 便携式显示器

    ##### Lepow Z1 便携式显示器

    一旦您习惯了双显示器设置，就很难回头了，因此便携式显示器是在任何地方获得类似体验的绝佳方式。这款 15.6 英寸显示器具有全高清分辨率和内置扬声器。它使用一根 USB-C 电缆连接，因此很容易设置，尽管它也有迷你 HDMI。

## 外部 GPU 外壳

惠普 Elite 蜻蜓 G3 在图形能力方面没有太多优势，因为它仅由处理器本身内置的英特尔 Iris Xe GPU 提供支持。对于商用笔记本电脑来说，这很好，但是如果您想做得更多呢？无论你是想玩一些游戏，还是有一些专业的创意工作要做，外部 GPU 都将大大有助于实现这一点。它们价格昂贵，并不适合所有人，但对于一些惠普精英蜻蜓 G3 用户来说，它们可能是重要的附件。

*   <picture>![The Mantiz Saturn pre II is another great option for an external GPU enclosure. It can deliver up to 550W of power to the GPU, 100W to your laptop, and it even has a SATA slot for an HDD or SSD. Plus, it has multiple USB ports and Ethernet to expand the connectivity on your laptop.](img/11ece4d7ce6b7aedabaa0bf832db58c3.png)</picture>

    Mantiz Saturn Pro II

    ##### Mantiz MZ-03 Saturn Pro II

    Mantiz Saturn Pro II eGPU 外壳是另一款外观时尚的设备，更适合专业用户。它可以适应大多数现代 GPU，并为它们提供高达 550 瓦的功率，另外为您的笔记本电脑提供 100 瓦的功率。它还包括五个 USB Type-A 端口、一个 SD 读卡器、千兆以太网和一个 SATA 驱动器插槽。

*   <picture>![The MasterCase EG200 is another solid external GPU, but in adddition to the GPU itself, it can also house an external SATA drive so you get extra storage. It has a 550W PSU, and it can deliver 60W of power to charge your laptop. It also serves as a stand to stow away your laptop.](img/da0bf7bfeb592b55b57935fa425d8b62.png)</picture>

    酷派大师大师 EG200

    ##### 酷派大师大师 EG200

    酷派大师 eg 200 不像其他一些产品那样令人印象深刻，但它有 550 瓦的 PSU，可以为您的笔记本电脑提供 60 瓦的功率。它有三个 USB 端口和一个 SATA 驱动器插槽，此外它还可以兼作笔记本电脑支架，这样您就可以将它放在一边。

## 惠普精英蜻蜓 G3 的坞站和适配器

像大多数商用笔记本电脑一样，惠普 Elite 蜻蜓 G3 开箱后有相当稳定的端口供应。你有两个 Thunderbolt 4 端口，一个 USB Type-A，HDMI 和一个耳机插孔。尽管如此，像这样的 13 英寸轻便笔记本电脑只能容纳这么多，如果你想要更多，这是完全可以理解的。我们已经为惠普精英蜻蜓 G3 选出了[最佳坞站，但这里还是有一些我们的首选:](https://www.xda-developers.com/best-docks-hp-elite-dragonfly-g3/)

*   <picture>![This Plugable dock is one of the most versatile ones out there, as it has a ton of ports. Six USB Type-A ports, two HDMI and two DisplayPort connections (two simultaneous outputs in total), Gigabit Ethernet, and SD card readers are all here, and it can even charge your laptop at up to 96W.](img/725e38e5ff16207f0e5bcda4a287b32d.png)</picture>

    可插拔 14 合 1 雷电 3 坞

    ##### 可插拔 14 合 1 雷电 3 坞

    这种可插拔坞是最通用的坞站之一，因为它有一吨的端口。六个 USB Type-A 端口，两个 HDMI 和两个 DisplayPort 连接(仅支持两个同步输出)，千兆以太网和 SD 读卡器都在这里，它甚至可以以高达 96W 的功率为您的笔记本电脑充电。

*   <picture>![If you can't stomach the price of other Thunderbolt docks, this alternative gives you plenty of expansion options, including DisplayPort and daisy-chaining support, at a much lower price.](img/6ddac4625becca49b73204fab0689572.png)</picture>

    亚马逊基础版 10 合 1 雷电 3 坞

    ##### 亚马逊基础版 10 合 1 雷电 3 坞

    如果你想要一个便宜的迅雷坞，这款来自亚马逊基础版的是一个很棒的选择。它有两个 USB 端口，HDMI 和 DisplayPort 输出，千兆以太网，甚至价格低廉的菊花链。它还可以为您的笔记本电脑提供高达 60W 的功率，因此它的功能仍然非常全面。

*   <picture>![Anker is a well-known and reliable brand when it comes to charging accessories and USB hubs. This one is a great option if you have a lot of Thunderbolt peripherals, since it gives you three downstream ports, plus USB Type-A for a low price.](img/91e57c19b141da81601fed491f740438.png)</picture>

    Anker power expand 5-in-1 Thunderbolt 4 Mini Dock

    ##### Anker power expand 5-in-1 Thunderbolt 4 Mini Dock

    如果你的设置中有很多 Thunderbolt 配件，这是帮助你连接它们的最佳 Dock。它为您提供了三个 Thunderbolt 4 端口，外加一个 USB Type-A 端口，用于您可能想要连接的外围设备。它还支持高达 85W 的功率输出。

*   <picture>![This versatile 11-in-1 docking station includes HDMI, DVI, microphone, headphone, and half a dozen USB-A ports. It's particularly useful if you seek a DVI port, as these are going extinct.](img/3567f51c5516c83b88e195131e07d177.png)</picture>

    可插拔坞站

    ##### 可插拔 USB 3.0 通用笔记本坞站

    几乎任何一款迅雷坞站都要花费 200 多美元，所以我们不会责怪你想要更便宜的东西。这个坞站有六个 USB Type-A 端口，HDMI，DVI，以太网，以及独立的耳机和麦克风插孔，所有这些都不到 100 美元。不过，它不支持充电供电。

*   <picture>![If you're planning to connect multiple displays to your laptop, this aluminum Totu Hub can be very useful. It packs two HDMI ports and a VGA port. The HDMI ports support up to 4K 30Hz output, whereas VGA supports up to 1080p. You can also get three USB 3.0 Type-A ports, an SD card slot, a microSD card slot, and a Type-C port for PD.](img/97e090aeb1fe4b397504df609dfb43ad.png)</picture>

    Totu 9 合 1 集线器

    ##### Totu 9 合 1 USB-C 集线器

    坞站最适合需要连接大量外围设备的办公环境，但有时您需要更多移动端口。这个集线器包括多个 USB 端口和显示输出，加上它有一个 SD 读卡器，它支持电源传输为您的笔记本电脑充电，所以它非常通用。

## 鼠标和键盘

设计笔记本电脑需要对我们通常在台式电脑上看到的东西进行一些改变。例如，笔记本电脑的键盘不会像独立的桌面键盘那样舒适。此外，笔记本电脑使用触摸板而不是传统的鼠标，因为这是笔记本电脑真正能做的事情。但是，如果您使用 HP Elite 蜻蜓 G3 启动桌面设置，合适的鼠标和键盘是非常重要的附件。以下是我们的一些建议:

*   <picture>![A split keyset design helps to position wrists and forearms in a natural, relaxed position. Cushioned palm rest provides support and promotes a neutral wrist position. You also get a separate number pad, perfect for spreadsheet work.](img/882cff441ff1cd5ffe2da59103d5fc13.png)</picture>

    微软雕塑键盘

    ##### 微软雕塑人体工程学键盘

    乍一看可能有点不太对劲，但微软雕塑键盘的设计有助于确保所有按键都更容易够到，而且你的手腕不会紧张，这要归功于键盘和手腕托的弧形形状。它还包括一个单独的数字键盘。

*   <picture>![Looking for a more satisfying typing exsperience? Mechanical keyboards may be for you, and the Razer Pro Type Ultra is a fantastic one for productivity. Its clean silver and white design is perfect for any workspace, and the silent linear switches are both comfortable and quiet for use in shared spaces.](img/0e602c2cf0acb1db0ca852a692dd9277.png)</picture>

    雷蛇 Pro Type Ultra

    ##### 雷蛇 Pro Type Ultra

    雷蛇制造了一些知名的游戏产品，但是 Pro Type Ultra 是一款非常有利于生产力的机械键盘。它使用 Razer 的静音线性开关，它有一个干净的白色设计和白色背光，因此它非常适合任何办公室。此外，它还有一个带软垫的腕托。

*   <picture>![Want a complete setup without fuss? This Logitech bundle includes a full-size keyboard with large comfortable keys, plus an ambidextrous mouse for basic use. The peripherals connect using a wireless dongle so you can be free of cables, and they last a long time on a charge.](img/c1a4daad793761a23432e7824b6d0425.png)</picture>

    罗技 MK270 二合一

    ##### 罗技 MK270 二合一

    想一口气搞定鼠标和键盘？这款组合套装包括一个无线鼠标和全尺寸键盘，因此您可以立即以低廉的价格准备好您的办公设置。他们没什么特别的，但是他们把工作做得很好。

*   <picture>![The HP 635 is a fairly basic mouse all things considered, but it looks sleek and has an ambidextrous design that should work for everyone. It has four customizable buttons and it can connect to up to three devices vie Bluetooth or the included dongle.](img/56c4d57a62d0cee4d530e2189b88bfe4.png)</picture>

    HP 635 无线鼠标

    ##### HP 635 无线鼠标

    从各方面考虑，HP 635 是一款相当基本的鼠标，但它看起来很时尚，并且具有灵巧的设计，应该适合所有人使用。它有四个可定制的按钮，可以通过蓝牙或随附的加密狗连接多达三个设备。

*   <picture>![Talking about the best mice without mentioning the Logitech MX Master 3S would be egregious. With a fast 8,000 DPI sensor, it even works on glass, it has a premium build with a metal scroll wheel (and one for horizontal scrolling), and it's super comfortable. Plus it supports Bluetooth or a wireless dongle.](img/c7f2765da242bbf758adf25a14d420c2.png)</picture>

    罗技 MX Master 3S

    ##### 罗技 MX Master 3

    罗技的 MX Master 3 是市场上最好的鼠标之一，具有 MagShift 滚轮(外加第二个滚轮用于水平滚动)，可定制的按钮和高 DPI 传感器。不过，它的设计是为右手使用者设计的。

## 耳机和耳塞

惠普表示，Elite 蜻蜓 G3 拥有带分立放大器的“高级扬声器”，虽然它们自己肯定能提供良好的体验，但使用扬声器并不总是理想的。没有人想买那个在公共场合大声放音乐给所有人听的人(至少我们希望如此)。如果您想在外出时使用 HP Elite 蜻蜓 G3 听一些东西，耳机和耳塞是很好的配件，因此我们有以下建议:

*   <picture>![The Apple AirPods Pro also support ANC and are water-resistant. They're available in only one color, though.](img/53e69157e96316bbef1298e5037d6014.png)</picture>

    苹果 ai rpods Pro

    ##### 苹果 ai rpods Pro

    ai rpods Pro 至今仍被认为是你能买到的最棒的无线耳塞之一，而且是最好的耳机。它们设计小巧，具有 ANC 和空间音频、自适应均衡器等功能。另外，Windows 11 正在针对 AirPods 进行优化。

*   <picture>![If you want something different from mst other earbuds, the Sennheiser Momentum True Wireless 2 are a great option. They feature 7mm drivers and ANC, but the design is also unique, featuring a fabric-covered case and premium-feeling earbuds in neutral colors that look great anywhere.](img/32ac020e224532522f3fd4572bebff4a.png)</picture>

    森海塞尔气势真无线 2

    ##### 森海塞尔气势真无线 2

    森海塞尔制造了一些非常受欢迎的音频产品，这也是一对很棒的耳塞。他们有 7 毫米的驱动器和高级功能，如 ANC。此外，他们有一个更经典的风格设计，从同行中脱颖而出。

*   <picture>![The Sony WH-1000XM4 are high-end Bluetooth headphones with a premium look and fantastic audio quality and ANC. At $228, they're a fantastic deal.](img/3432a496e184198026cd1c60349cef0e.png)</picture>

    索尼 WH-1000XM4(121 美元起)

    ##### 索尼 WH-1000XM4

    索尼是音频领域另一个极其受欢迎的品牌，WH-1000 xm4 绝对不会错。它们具有双噪声传感器技术的 ANC、磨损检测等功能。它们也有三种高级外观的款式。

*   <picture>![The Surface Headphones 2 are a bit older, but they still deliver great audio quality and ANC. They have intuitive controls via the dials built into the earcups. Plus, the Surface app for Windows lets you update and configure them.](img/c3c66a6fa933eed22d0ebaad518aab6c.png)</picture>

    Surface Headphones 2

    ##### 微软 Surface Headphones 2

    Surface Headphones 2 是一款出色的高级耳机，采用直观的设计，使用拨号控制来微调音量和 ANC 水平。此外，它们被设计为与 Windows 一起工作，并且它们有一个专用的应用程序，这是你不常看到的。

*   <picture>![Want something a bit easier on the wallet? The SteelSeries Arctis 1 is a great headset for gaming and communication, featuring a detachable microphone and a clean and sleek design. It uses a wired connection so you don't have to worry about battery, and it has built-in volume controls and a mute switch on the earcup.](img/a5319149b167290135caebf220bdcfe5.png)</picture>

    steel series Arctis 1

    ##### steel series Arctis 1

    如果你不想担心电池问题，这款有线耳机可能是你的理想选择。这是一个游戏耳机，但它有一个非常干净的设计，在任何地方都可以很好地工作。麦克风是可拆卸的，所以你只在需要的时候使用它，它不会碍事。

## 面向惠普精英蜻蜓 G3 的网络摄像头

说实话，惠普精英版蜻蜓 G3 不太可能需要新的网络摄像头。惠普为它配备了一个 500 万像素的摄像头，可以拍摄 1080p 的视频，此外，它还通过自动取景和光线校正等惠普在线功能得到了增强。对于大多数用例来说，这应该是一个很好的网络摄像头，但也有一些情况下，你可能想要更好的东西。以下是我们推荐的一些[最佳网络摄像头](https://www.xda-developers.com/best-webcams/):

*   <picture>![The XPS 17 has a pretty bad webcam, so you're going to want a better camera if you plan to take video calls or stream online. The Dell UltraSharp is possibly the best webcam on the market, with a 4K Sony STARVIS sensor for low-light, autofocus, and other great features.](img/2c5030ef0fd3d7424d0f962ac40c2bb6.png)</picture>

    戴尔 UltraSharp 4K 网络摄像头

    ##### 戴尔 UltraSharp 4K 网络摄像头

    如果精英蜻蜓 G3 上的内置网络摄像头不够好，戴尔 UltraSharp 4K 可能是目前市场上最好的网络摄像头。凭借 4K 索尼 STARVIS 传感器，它具有出色的弱光性能，还具有自动对焦、自动取景和灯光调节功能。它甚至包括 Windows Hello 支持。

*   <picture>![The Anker PowerConf C302 is a great webcam if you're participating in calls with a big group thanks to its wide field of view. Plus, you can adjust its position, and it features solid webcam quality at 2K and 30fps, along with some AI features to improve image quality.](img/33f55a3c8efb7fb134a3156f24a53721.png)</picture>

    Anker PowerConf C302

    ##### Anker PowerConf C302 网络摄像头

    您是否正在与房间中的其他人通话？Anker PwerConf C302 是一款出色的广角相机，非常适合在一个画面中拍摄一群人。它拥有 115 度的视野和 2K 分辨率，以及自动取景、自动对焦等功能。您还可以轻松调整网络摄像头的位置，使其恰到好处。

## 惠普精英蜻蜓 G3 的案例

如果你正在购买一台像惠普精英蜻蜓 G3 这样的高端笔记本电脑，你可能会同意，一个保护它安全的外壳是你能得到的最重要的配件之一。这是一台昂贵的笔记本电脑，所以你要确保它能尽可能长时间使用，一个箱子或袋子肯定会有所帮助。我们已经收集了惠普精英蜻蜓 G3 的[最佳案例](https://www.xda-developers.com/best-cases-hp-elite-dragonfly-g3/)是一篇独立的文章，但这里有一些来自那里的顶级精选:

*   <picture>![The Nillkin sleeve is an interesting case, as not only does it offer protection for your HP EliteBook 840 G9, but it also doubles as a laptop stand and the flap can be used as a mouse pad for extra precision.](img/a2f714151daa1217e2db3283a44d3e68.png)</picture>

    Nillkin 笔记本电脑套

    ##### Nillkin 笔记本电脑套带支架

    这款时尚的 Nillkin 套不占太多空间，并且有几种颜色可供选择。此外，它还可以兼作笔记本电脑支架，打开的翻盖可以用作鼠标垫，所以它不仅仅是一个箱子。

*   <picture>![The Tomtoc 360 bag is a modern-looking case that offers more protection than you may think. It has reinforced corners and a lot of cushioning on the inside to keep your laptop safe from drops and bumps. Plus, it looks great.](img/5f5682f12daeb73c2564b50e756e23cd.png)</picture>

    Tomtoc 360 保护套

    ##### Tomtoc 360 保护型笔记本电脑单肩包

    Tomtoc 360 包看起来很干净，可以随身携带，但它提供的保护比它看起来的更多。边角经过加固，内部有大量缓冲垫，可保护您的笔记本电脑免受跌落和碰撞。

## 外部存储器

你可以获得高达 2TB 内部存储的三星 Galaxy Book 2 Pro 系列，但该选项与你可能想要或不想要的其他升级相关联。如果您希望获得更便宜的配置并在以后扩展您的存储，或者如果您只是需要移动文件，这些都是您的绝佳选择:

*   <picture>![If you're worried about misplacing your cable, this SSD has it built right in. It also supports Thunderbolt, though speeds only go up to 2,400MB/s reads and 1,800 MB/s writes, which is still faster than most.](img/329a715ace99132cdaea79e59ebd0efd.png)</picture>

    可插拔的雷电 3 SSD

    ##### 可插拔的雷电 3 外置 SSD

    Thunderbolt 线缆相当昂贵，所以如果你不想冒失去你的 SSD 的风险，这款 SSD 是一个有效的选择。它的速度非常快，高达 2，400MB/s，容量高达 2TB。它也是由金属制成的，所以不容易破碎，散热更有效。

*   <picture>![SanDisk's Extreme Pro SSD combines fast speeds (up to 2,000MB/s) with an extra-rugged design for a device that you can truly take anywhere. The aluminum chassis is designed to withstand drops up to two meters and it has IP55 water and dust resistance.](img/b2883c3635948d37082254100ae4a408.png)</picture>

    SanDisk Extreme Pro SSD

    ##### SanDisk Extreme Pro 便携 SSD

    509.99Save $ 290

    你需要一款可以承受打击的 SSD 吗？SanDisk Extreme PRO 是周围最好的坚固固态硬盘之一，具有 IP55 等级，2 米跌落保护和坚固的铝制机箱。不是 Thunderbolt，但还是支持高达 2000 MB/s 的速度

*   <picture>![There are many ways to keep your data safe, but the Samsung T7 Touch SSD makes it easier since it allows you to unlock the data inside using the fingerprint sensor. It's not as fast some Thunderbolt SSDs, but it's still far from slow.](img/e77fb3919072a9ef1786c2922c223b99.png)</picture>

    三星 T7 Touch

    ##### 三星 T7 Touch 便携式固态硬盘

    想让您的数据既安全又易于访问？这款三星固态硬盘配有指纹识别器，只需手指一触，即可轻松保护和解锁文件。此外，速度高达 1，050MB/s，即使没有 Thunderbolt，也仍然非常快。

*   <picture>![Most people may want fast storage, but sometimes, you just want a massive backup. And this massive Seagate HDD comes in capacities up to 18TB, so you can store just about all your files for a much lower price than you'd pay for the same amount of SSD storage.](img/8085d0abf2d3f8e1c1f4cc6485434ce5.png)</picture>

    希捷扩展 HDD

    ##### 希捷扩展 HDD

    SSD 的速度很棒，但也很贵，而且不容易找到尺寸超过 4TB 的 SSD。如果你想要一个大容量的备份存储，希捷扩展固态硬盘可能是你需要的。它的容量高达 18TB，价格不到 500 美元。

*   <picture>![This tiny flash drive is the most convenient way to share files in a pinch since you can easily slip it into a pocket next to something else. It has USB-A and USB-C connections, so it works on computers, tablets, and most Android phones.](img/eab3467d436821e848e28bd83bec990d.png)</picture>

    金士顿 micro duo 3C

    ##### 金士顿 microDuo 3C

    固态硬盘已经相当便携，但是如果你想要一个可以轻松放在口袋里的东西，这款金士顿 drive 就是要走的路。它有 USB Type-A 和 Type-C 端口，因此它可以与 PC 和智能手机一起工作，并且它的大小高达 128GB。

## 电光队

与智能手机不同，像惠普精英版蜻蜓 G3 这样的高端笔记本电脑仍然包括充电器，所以这不是每个人都需要的配件之一。但是，如果你不幸丢失了笔记本电脑的充电器，你可能知道找到一个替代品有多难。大多数手机和笔记本电脑都支持 USB-C 充电，但电压仍然存在差异，这使得更难找到实际工作的东西。如果您需要更换充电器，我们推荐以下选项:

*   <picture>![This 4-port charger has a massive 108W of power output, and it can deliver up to 96W to your laptop alone. That will go down to 65W if you use all the ports, but it's still very impressive for this kind of charger.](img/753dcc33571c811c30a50fa1524c6b61.png)</picture>

    Belkin 108 w 4 口充电器

    ##### Belkin boost charge Pro 108 w GaN 4 口充电器

    这款 Belkin 可能有点大，但那是因为它的四个端口可以让你一次给所有设备充电。主端口最高支持 96W 充电，即使连接其他设备，也高达 65W。这是你在旅途中唯一需要的充电器。

*   <picture>![This set won't deliver the recommended power for the Core i7 Surface Laptop Studio, but if you have a Core i5 model, it can keep it charged whether you're at home or on the go with the included power bank.](img/5bdfd13c970694adbf852743d292e0e4.png)</picture>

    Anker 737 电源组

    ##### Anker 737 Power core 26K

    此套装不仅可以让您在家中使用 65W 壁式充电器为笔记本电脑充电，还可以使用 20，000mAh 电源组为您的笔记本电脑提供高达 60W 的功率

## 其他配件

到目前为止，我们已经涵盖了您可能感兴趣的大多数类别。然而，还有一些您可能会对惠普精英蜻蜓 G3 感兴趣的附件，尽管它们并不完全符合上述类别。因此，我们把它们留在下面，以防你觉得它们有趣:

*   <picture>![We've already covered some audio products, but the Razer Anzu are wholly unique. These glasses have speakers built into the arms, and they come with two pairs of lenses - one for filtering blue light from your laptop and one for filtering sunlight while you're outside. ](img/1fe71893bb26198cd1e308a9237a6c90.png)</picture>

    雷蛇 Anzu 智能眼镜

    ##### 雷蛇 Anzu 智能眼镜

    这几乎可以归入我们的音频范畴，但却是一款完全独特的产品。这些眼镜有助于保护你的眼睛免受蓝光伤害，另外你还可以得到太阳镜镜片。它们的手臂上有微型扬声器，所以你可以用它们来听音乐。

*   <picture>![One of the most basic things you probably want to do with any laptop is keep it clean, and this cleaning kit is a basic, but effective solution to do just that. You get a large spray bottle (160z) of cleaning solution and a big microfiber cloth, so it can clean your tablet, and even bigger screens like TVs.](img/0a8c56009397a2d85841b64db7cbf0e6.png)</picture>

    屏幕妈妈屏幕清洁剂套装

    ##### 屏幕妈妈屏幕清洁剂

    有没有随着时间的推移收集到您笔记本电脑上的指纹、灰尘和其他污垢？Screen Mom 屏幕清洁套件为您提供了适用于精致屏幕的液体喷雾解决方案和超细纤维布，让您的笔记本电脑保持整洁。

* * *

如果你还没有，你可以使用下面的链接购买惠普精英蜻蜓 G3。如果你认为它不适合你，你可以看看你现在能买到的最好的惠普笔记本电脑。这些配件中的许多对他们来说也同样适用。

 <picture>![The HP Elite Dragonfly G3 still weighs in at under a kilogram, also packing all of the right features to make it the best laptop on the go.](img/838a8aad4740df2f46bd439d513904db.png)</picture> 

HP Elite Dragonfly G3

##### 惠普精英蜻蜓 G3

惠普 Elite 蜻蜓 G3 是惠普最高端的商用笔记本电脑，采用第 12 代英特尔处理器、3:2 显示屏和轻量级设计。